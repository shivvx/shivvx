...
USER_NAME = os.environ.get('USER_NAME', 'shivvx')
...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None):
    ...
    request = requests.post('https://api.github.com/graphql', json={'query': query, 'variables': variables}, headers=HEADERS)
    if request.status_code == 200:
        if request.json().get('data', {}).get('repository', {}).get('defaultBranchRef') is not None:
            return loc_counter_one_repo(owner, repo_name, data, cache_comment, request.json()['data']['repository']['defaultBranchRef']['target']['history'], addition_total, deletion_total, my_commits)
        else:
            return 0, 0, 0
    force_close_file(data, cache_comment)
    if request.status_code == 403:
        raise Exception('Too many requests in a short amount of time!\nHit anti-abuse limit!')
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=None):
    ...
    if page_info['hasNextPage']:
        ...
        return cache_builder(edges + curr_edges, comment_size, force_cache)
...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    ...
    for index in range(len(edges)):
        ...
        if repo_hash == hashlib.sha256(edges[index]['node']['nameWithOwner'].encode('utf-8')).hexdigest():
            try:
                target_ref = edges[index]['node'].get('defaultBranchRef')
                if target_ref and int(commit_count) != target_ref['target']['history']['totalCount']:
                    owner, repo_name = edges[index]['node']['nameWithOwner'].split('/')
                    loc = recursive_loc(owner, repo_name, data, cache_comment)
                    data[index] = repo_hash + ' ' + str(target_ref['target']['history']['totalCount']) + ' ' + str(loc[2]) + ' ' + str(loc[0]) + ' ' + str(loc[1]) + '\n'
            except Exception:
...
if __name__ == '__main__':
    print('Starting Profile Stats Sync for @' + USER_NAME)
    
    # Birthdate for uptime calculation
    birthday = datetime.datetime(2004, 12, 11)
    ...
    if ACCESS_TOKEN:
        try:
            ...
            print('Successfully updated SVGs via GitHub GraphQL API!')
        except Exception as e:
            print(f'GraphQL update note: {e}')
            svg_overwrite('dark_mode.svg', age_data, 386, 12, 17, 22, 20, ['104,220', '11,740', '92,480'])
            svg_overwrite('light_mode.svg', age_data, 386, 12, 17, 22, 20, ['104,220', '11,740', '92,480'])
            print('Updated uptime in SVGs via fallback mode.')
    else:
        svg_overwrite('dark_mode.svg', age_data, 386, 12, 17, 22, 20, ['104,220', '11,740', '92,480'])
        svg_overwrite('light_mode.svg', age_data, 386, 12, 17, 22, 20, ['104,220', '11,740', '92,480'])
        print('ACCESS_TOKEN not found: updated SVGs with current calculated uptime & baseline metrics.')
